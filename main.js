...
function entrenarPerceptron() {
  ...
  while (true) {
    ...
    iteracion++;
    console.log(`Iteración ${iteracion}, errores: ${contadorDeErrores}`);
...
function probarPerceptron() {
  const entradasConSalidaEsperada = [
    [[1, 6.1, 2.8], 0], [[1, 6.3, 2.5], 0], [[1, 6.1, 2.8], 0],
    [[1, 6.4, 2.9], 0], [[1, 6.6, 3.0], 0], [[1, 6.8, 2.8], 0],
    [[1, 6.7, 3.0], 0], [[1, 6.0, 2.9], 0], [[1, 5.0, 3.4], 1],
    [[1, 5.7, 2.6], 0], [[1, 5.5, 2.4], 0], [[1, 5.5, 2.4], 0],
    [[1, 5.8, 2.7], 0], [[1, 6.0, 2.7], 0], [[1, 5.4, 3.0], 0],
    [[1, 6.0, 3.4], 0], [[1, 6.7, 3.1], 0], [[1, 6.3, 2.3], 0],
    [[1, 5.6, 3.0], 0], [[1, 4.4, 2.9], 1], [[1, 5.5, 2.5], 0],
    [[1, 5.5, 2.6], 0], [[1, 6.1, 3.0], 0], [[1, 5.8, 2.6], 0],
    [[1, 5.0, 2.3], 0], [[1, 5.6, 2.7], 0], [[1, 5.7, 3.0], 0],
    [[1, 5.7, 2.9], 0], [[1, 6.2, 2.9], 0], [[1, 5.1, 2.5], 0]
  ];
...
function evaluarDesempeño(entradasEsperadas) {
  ...
  entradasEsperadas.forEach(([entrada, salidaEsperada], index) => {
    ...
    console.log(`Entrada ${index + 1}: [${entrada.slice(1)}] → ` +
                `Predicho: ${salidaPredicha} (${salidaPredicha === 1 ? "Setosa" : "Otro"}) ` +
